...
for g in H:
	...
	for index in Index[k-1]:
		...
		U=matrix(GF(2),X);
		V=U.transpose();		
		fname = f'/home/dilip/Desktop/6-bit_Sbox/supplementary_material/verilogcode/SMSS32_5/SMSS32_62_nn_{k}_{l}.v'
		with open(fname, "w") as f:
			f.write("`timescale 1ns/100ps\n");
			f.write("module SMSS32_62_nn_{}_{}(x,y);\n".format(k,l));
			f.write("\t input [5:0] x;\n");
			...
			f.write("\t wire [2:0] t;\n");
			f.write("\t output [2:0] c;\n");
			f.write("\t assign t[0]=(a[0]&b[1])^(a[1]&b[0]);\n");
			f.write("\t assign t[1]=(a[1]&b[2])^(a[2]&b[1]);\n");
			f.write("\t assign t[2]=(a[0]&b[2])^(a[2]&b[0]);\n");
			f.write("\t assign c[0]=(a[2]&b[2])^t[0]^t[1];\n");
			f.write("\t assign c[1]=(a[0]&b[0])^t[1]^t[2];\n");
			f.write("\t assign c[2]=(a[1]&b[1])^t[2]^t[0];\n");
			f.write("endmodule\n");
			f.write("\n")
			...
			f.write("\t assign b[1]=a[1]^(~a[2]&a[0]);\n");
			f.write("\t assign b[2]=a[2]^(~a[0]&a[1]);\n");
			f.write("endmodule\n");
			f.write("\n")
